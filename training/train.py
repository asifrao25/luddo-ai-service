...
def main():
    ...
    parser.add_argument('--base-model', required=True, help='Base model name (e.g., llama3.2:3b)')
    parser.add_argument('--dataset', required=True, help='Path to JSONL dataset')
    parser.add_argument('--output', required=True, help='Output model name')
    parser.add_argument('--epochs', type=int, default=3, help='Number of training epochs')
    parser.add_argument('--progress-file', required=True, help='File to write progress updates')
    parser.add_argument('--output-dir', required=True, help='Directory for output model')
        ...
        for line in process.stdout:
            ...
            if 'Iter' in line and 'Train loss' in line:
                try:
                    ...
                    current_iter = int(iter_match.split()[1])
...
SYSTEM """You are an expert Luddo game AI. You have been fine-tuned on thousands of winning game decisions.

When analyzing moves, consider:
1. Capture opportunities (highest priority)
2. Safety from opponent captures
3. Advancement toward home
...
'''
        ...
        if result.returncode != 0:
            ...
            # Try alternative: create from the fused model directory
            alt_modelfile = f'''FROM {fused_path}

SYSTEM """You are an expert Luddo game AI. Analyze board positions and select optimal moves."""
PARAMETER temperature 0.7
'''
            with open(modelfile_path, 'w') as f:
                f.write(alt_modelfile)

            subprocess.run(['ollama', 'create', args.output, '-f', modelfile_path])
